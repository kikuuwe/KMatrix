...
void iniU_squVOVT(KUSymMat<T,N>* const pOut, const KVector<T,N>& A)
{
	for(int i=0;i<N;i++)
	{
		double& Ai= A(i);
...
template<class T,int N> inline
void minU_squVOVT(KUSymMat<T,N>* const pOut, const KVector<T,N>& A)
{
	for(int i=0;i<N;i++)
	{
		double& Ai= A(i);
...
template<class T,int N> inline
void minU_squVOSVT(KUSymMat<T,N>* pOut, const KVector<T, N>& A, const T& a)
{
	for(int i=0;i<N;i++)
	{
		double t = (A(i))* a;
...
template<class T,int N> inline void
iniS_mulVTUOVO(T* const pOut,const KVector<T,N>& A,const KUSymMat<T,N>& B,const KVector<T,N>& C){___xxxS_mulVTUOVO___( = , += );}

template<class T,int N> inline void
addS_mulVTUOVO(T* const pOut,const KVector<T,N>& A,const KUSymMat<T,N>& B,const KVector<T,N>& C){___xxxS_mulVTUOVO___( += , += );}

template<class T,int N> inline void
minS_mulVTUOVO(T* const pOut,const KVector<T,N>& A,const KUSymMat<T,N>& B,const KVector<T,N>& C){___xxxS_mulVTUOVO___( -= , -= );}

#undef ___xxxS_mulVTUOVO___

/**********************************************************************************/

#define ___xxxS_quadVTUOVO___( _eqsini_, _eqs_ ) {                          \
  const T* pA = (const T*)&A;                                               \
  const T* pB = (const T*)&B;                                               \
  const T* pC = (const T*)&A;                                               \
	{                                                                         \
		                                 (*pOut)_eqsini_    (*pA)*(*pB)*(*pC);  \
		for(int j=1  ;j<N;j++){pC++;pB++;(*pOut)_eqs_    2.*(*pA)*(*pB)*(*pC);} \
	}                                                                         \
	for(int i=1;i<N;i++)                                                      \
	{                                                                         \
		pA++;pC =pA;pB+=(i+1);                                                  \
		                                 (*pOut)_eqs_       (*pA)*(*pB)*(*pC);  \
		for(int j=i+1;j<N;j++){pC++;pB++;(*pOut)_eqs_    2.*(*pA)*(*pB)*(*pC);} \
	}                                                                         \
}                                                                           

template<class T,int N> inline
void iniS_quadVTUOVO(T* const pOut,  const KUSymMat<T,N>& B, const KVector<T, N>& A){___xxxS_quadVTUOVO___( = ,+=);}

template<class T,int N> inline
void addS_quadVTUOVO(T* const pOut,  const KUSymMat<T,N>& B, const KVector<T, N>& A){___xxxS_quadVTUOVO___(+= ,+=);}

template<class T,int N> inline
void minS_quadVTUOVO(T* const pOut,  const KUSymMat<T,N>& B, const KVector<T, N>& A){___xxxS_quadVTUOVO___(-= ,-=);}

#undef ___xxxS_quadVTUOVO___
